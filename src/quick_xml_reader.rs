    ...
    F: FnMut(Node, T) -> T,
{
    ...
    let mut acc = initial;
    loop {
        let event = parser.read_event_into(&mut buffer)?;
        match event {
    ...
    F: FnMut(Node, T) -> T,
{
    ...
    loop {
        let event = parser.read_event_into(buffer)?;
        match event {
            Event::Start(ref e) => match e.name().as_ref() {
                b"node" => {
                ...
                b"note" | b"way" | b"relation" => {
                    parser.read_to_end_into(e.name(), &mut inner_buffer)?;
                    inner_buffer.clear();
                }
                _ => {
                    return Err(ScanError::Other(format!(
                        "An unexpected start element: {:?}",
                        from_utf8(e.name().as_ref())
                    )));
                }
            },
...
fn read_node<'e, R: std::io::BufRead>(
    ...
    for a_res in event.attributes() {
        ...
        match a.key.as_ref() {
            b"id" => {
                let value = a.decode_and_unescape_value(parser)?;
...
fn read_tag<'e, R: std::io::BufRead>(
    parser: &mut Reader<R>,
    event: &BytesStart<'e>,
) -> Result<Tag, ScanError> {
    ...
    for a_res in event.attributes() {
        ...
        match a.key.as_ref() {
            b"k" => {
                let value = a.decode_and_unescape_value(parser)?;
                key_opt = Some(value.into_owned());
            }
            b"v" => {
                let value = a.decode_and_unescape_value(parser)?;
                value_opt = Some(value.into_owned());
            }
            _ => (), // ignore other fields
        }
    }
